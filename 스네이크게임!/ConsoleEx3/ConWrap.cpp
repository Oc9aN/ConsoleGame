...
void PutFPS()
{
	...
	char msg[80] = ""; sprintf_s(msg, "FPS=%.2f", fps);
...
BOOL DrawChar(int x, int y, char c, WORD fcolor, WORD bcolor)
{
	...
	{
#ifdef  _DEBUG
 		char msg[80]; sprintf_s(msg, "* ConWrap Ex3.0 : 경고! *  DrawChar() 좌표 오류 : x:%d\ty=%d\n", x, y );
...
//클립핑 코드 필요.
	...
	{
#ifdef  _DEBUG
 		char msg[80]; sprintf_s(msg, "* ConWrap Ex3.0 : 경고! *  DrawCharEx3() 좌표 오류 : x:%d\ty=%d\n", x, y );
		OutputDebugString(msg);
...
BOOL DrawStr(int x, int y, char* str, WORD fcolor, WORD bcolor)
{
	...
	{
#ifdef  _DEBUG
 		char msg[80]; sprintf_s(msg, "* ConWrap Ex3.0 : 경고! *  DrawStr() 좌표 오류 : x:%d\ty=%d\n", x, y );
		OutputDebugString(msg);
...
void DrawStrEx3(int x, int y, char* str, WORD fcolor, WORD bcolor)
{
	...
	{
#ifdef  _DEBUG
 		char msg[80]; sprintf_s(msg, "* ConWrap Ex3.0 : 경고! *  DrawStrEx3() 좌표 오류 : x:%d\ty=%d\n", x, y );
		OutputDebugString(msg);
	...
	else if( x < 0)	//x축 0 보다 작은 경우..클립핑...
	{
		...
		if( old_len <= abs(x) )		//출력할 문자열길이 보다 출력 좌표가 더 마이너스 값이라면... 
			return;
		strcpy_s(temp, str + abs(x));   //temp += (x);				//문자열 주소 보정..
 		x = 0;						//출력 좌표 보정.
	}
...
#ifdef _DEBUG
	char msg[80]; sprintf_s(msg, "p:0x%08x, b:0x%08x", g_pPriBuffer, g_pBkBuffer);
...
#ifdef _DEBUG
	{
		sprintf_s(msg, "** ConWrapper : Change Code Page. 949 : 한글 모드로 전환..\n");
#endif
...
#ifdef _DEBUG
	{
		sprintf_s(msg, "** ConWrapper : Change Code Page. 437 : 영문 모드로 전환..\n");
#endif
